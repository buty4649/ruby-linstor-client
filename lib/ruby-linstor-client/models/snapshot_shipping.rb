  ...
  class SnapshotShipping
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::SnapshotShipping` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::SnapshotShipping`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'from_node')
        self.from_node = attributes[:'from_node']
      end

      if attributes.key?(:'from_nic')
        self.from_nic = attributes[:'from_nic']
      end
      ...
      if attributes.key?(:'to_nic')
        self.to_nic = attributes[:'to_nic']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @from_node.nil?
        invalid_properties.push('invalid value for "from_node", from_node cannot be nil.')
      end
    ...
    def hash
      [from_node, from_nic, to_node, to_nic].hash
    end
