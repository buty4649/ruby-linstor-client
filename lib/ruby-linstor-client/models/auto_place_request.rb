  ...
  class AutoPlaceRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::AutoPlaceRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::AutoPlaceRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'layer_list')
        if (value = attributes[:'layer_list']).is_a?(Array)
          self.layer_list = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @select_filter.nil?
        invalid_properties.push('invalid value for "select_filter", select_filter cannot be nil.')
      end
    ...
    def valid?
      return false if @select_filter.nil?
      true
    end
    ...
    def hash
      [diskless_on_remaining, select_filter, layer_list].hash
    end
