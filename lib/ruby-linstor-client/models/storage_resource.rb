...
module LinstorClient
  class StorageResource
    ...
    def self.openapi_types
      {
        :'storage_volumes' => :'Array<StorageVolume>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::StorageResource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::StorageResource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'storage_volumes')
        if (value = attributes[:'storage_volumes']).is_a?(Array)
          self.storage_volumes = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          storage_volumes == o.storage_volumes
    end
