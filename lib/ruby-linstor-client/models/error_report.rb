...
module LinstorClient
  class ErrorReport
    ...
    # Filename of the error report on the server.  Format is: ```ErrorReport-{instanceid}-{nodeid}-{squencenumber}.log``` 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::ErrorReport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::ErrorReport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @error_time.nil?
        invalid_properties.push('invalid value for "error_time", error_time cannot be nil.')
      end
    ...
    def valid?
      return false if @error_time.nil?
      true
    end
