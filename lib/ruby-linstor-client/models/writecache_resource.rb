...
module LinstorClient
  class WritecacheResource
    attr_accessor :writecache_volumes

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'writecache_volumes' => :'writecache_volumes'
      }
    end
    ...
    def self.openapi_types
      {
        :'writecache_volumes' => :'Array<WritecacheVolume>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::WritecacheResource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::WritecacheResource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'writecache_volumes')
        if (value = attributes[:'writecache_volumes']).is_a?(Array)
          self.writecache_volumes = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          writecache_volumes == o.writecache_volumes
    end
