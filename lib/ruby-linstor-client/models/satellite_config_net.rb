...
module LinstorClient
  class SatelliteConfigNet
    attr_accessor :bind_address

    attr_accessor :port

    attr_accessor :com_type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::SatelliteConfigNet` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::SatelliteConfigNet`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'bind_address')
        self.bind_address = attributes[:'bind_address']
      end
      ...
      if attributes.key?(:'com_type')
        self.com_type = attributes[:'com_type']
      end
    end
    ...
    def hash
      [bind_address, port, com_type].hash
    end
