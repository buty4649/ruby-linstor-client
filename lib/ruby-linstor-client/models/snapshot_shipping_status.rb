...
module LinstorClient
  class SnapshotShippingStatus
    attr_accessor :snapshot

    attr_accessor :from_node_name

    attr_accessor :to_node_name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::SnapshotShippingStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::SnapshotShippingStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'from_node_name')
        self.from_node_name = attributes[:'from_node_name']
      end

      if attributes.key?(:'to_node_name')
        self.to_node_name = attributes[:'to_node_name']
      end
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('String', ["running", "complete"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["running", "complete"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          snapshot == o.snapshot &&
          from_node_name == o.from_node_name &&
          to_node_name == o.to_node_name &&
          status == o.status
    end
    ...
    def hash
      [snapshot, from_node_name, to_node_name, status].hash
    end
