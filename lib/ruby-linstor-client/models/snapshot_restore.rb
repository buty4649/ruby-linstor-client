  ...
  class SnapshotRestore
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinstorClient::SnapshotRestore` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinstorClient::SnapshotRestore`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'to_resource')
        self.to_resource = attributes[:'to_resource']
      end
    ...
    def list_invalid_properties
      ...
      if @to_resource.nil?
        invalid_properties.push('invalid value for "to_resource", to_resource cannot be nil.')
      end
    ...
    def valid?
      return false if @to_resource.nil?
      true
    end
